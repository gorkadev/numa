...
| Persisted Turn Metadata Reflects Full Turn | Thread persistence after phased turn | `message-meta.ts`/`withTurnMeta` consume the same `TurnCost` (unit 1b file list) | ✅ (column push confirmed 2026-09-11) |
| Ledger and Polar Reflect Full Turn | Ledger entry for phased turn | `usage.ts:79-174` `recordTurnUsage(cost: TurnCost)` inserts summed tokens/cost/`usageBreakdown` | ✅ (column push confirmed 2026-09-11) |
| Exactly One Polar Event Per Turn | Turn dispatches multiple sub-agents | `usage.ts:176-182` — one `ingestTurnCredits` call per `recordTurnUsage` invocation; `externalId` scheme unchanged (design decision 10) | ✅ (column push confirmed 2026-09-11) |
| Exactly One Polar Event Per Turn | Turn with failed sub-agent still ingests once | `cost.breakdown` includes the failed sub-agent's usage; still exactly one `ingestTurnCredits` call | ✅ (column push confirmed 2026-09-11) |
| Per-Sub-Agent Breakdown Retained | Breakdown available after phased turn | `usage.ts:162-170` — `usageBreakdown: {tier, entries}` stored in `turn_usage.usage_breakdown jsonb` | ✅ (column push confirmed 2026-09-11) |
...
| 9–10 (usage/ledger) | ✅ Yes | Per-run pricing summed once; one `turn_usage` row per turn, `usage_breakdown jsonb` added nullable via `pnpm db:push` (push confirmed via psql 2026-09-11) |
...
**WARNING**:
1. ~~`pnpm db:push` was never confirmed applied~~ **Resolved (orchestrator correction):** the push for the `usage_breakdown jsonb` column (`packages/db/src/schema.ts:234`) was re-run by the user after the unit 1b credential failure, and the column was confirmed read-only via psql on 2026-09-11 (`turn_usage.usage_breakdown` exists, `jsonb`) before unit 1c began. Residual note only: `recordTurnUsage`'s `catch` (`apps/web/lib/games/usage.ts:190-192`) swallows INSERT failures silently, so any future environment that misses a push would lose ledger rows and Polar ingestion without a visible signal.
...
**PASS WITH WARNINGS** — 61/61 tasks complete, 0 unchecked, 0 CRITICAL findings; both required verification commands (`tsc --noEmit`, `pnpm lint`) plus the `@workspace/db` typecheck pass clean; every spec requirement across all 7 capabilities has direct code/traceability evidence, with 6 scenarios confirmable only by exercising exact prompt-driven or as-yet-unexercised behavior (none contradicted) and 1 scenario dormant by the registry's own current population. WARNING-1 is resolved (the `usage_breakdown` push was confirmed via psql on 2026-09-11); the remaining warnings are either already-accepted (size exceptions), already-known (sub-agent view visual bugs), dormant (WARNING-2), prompt-level by design (WARNING-4), or a documented, non-blocking design/tasks-artifact gap (WARNING-3).
